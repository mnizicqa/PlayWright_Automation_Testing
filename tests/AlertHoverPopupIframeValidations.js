...
test(
  "@Web Alert,Hover,Popup,Iframe Validations",
  { tag: "@Web" },
  async ({ page }) => {
    ...
    await expect(iframePage.locator("h1")).toHaveText(
...
test("@Web Screenshot Capturing", { tag: "@Web" }, async ({ page }) => {
  await page.goto("https://rahulshettyacademy.com/AutomationPractice/");
  await expect(page.locator("#displayed-text")).toBeVisible();
  await page
...
test.skip("Visual Testing", { tag: "@Web" }, async ({ page }) => {
